...
const platform = readFileSync(new URL('../main/ios_memory.cpp', import.meta.url), 'utf8')

assert.match(platform, /Memory::reserveInternalDma\(std::size_t\)/, 'ios_memory.cpp must define reserveInternalDma')
assert.match(platform, /Memory::releaseInternalDma\(void \*\)/, 'ios_memory.cpp must define releaseInternalDma')
