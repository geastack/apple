...
const platform = readFileSync(new URL('../main/macos_sensors.cpp', import.meta.url), 'utf8')

assert.match(platform, /Memory::reserveInternalDma\(std::size_t\)/, 'macos_sensors.cpp must define reserveInternalDma')
assert.match(platform, /Memory::releaseInternalDma\(void \*\)/, 'macos_sensors.cpp must define releaseInternalDma')
